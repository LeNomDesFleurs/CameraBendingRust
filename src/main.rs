...
fn main() -> anyhow::Result<()> {
    ...
    filter_r.set_frequence_and_resonance(20000.0, 0.9);
    ...
    let mut random_generator = Random::new_min_max(0.0, 100.0, 0.99);
    ...
    for pixel in bufimg.enumerate_pixels_mut() {
        ...
        columns[pixel.0 as usize].push(pixel.2[color] as f32 + random_generator.process());
    ...
    for column in columns.iter_mut() {
        ...
        for (idx, sample) in column.iter_mut().enumerate() {
            if idx % 3== 1 {
                *sample = *sample - 200.0;
            } else {
                *sample = *sample + 200.0;
